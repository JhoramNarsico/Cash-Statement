...
class IntegratedCashFlowApp:
    ...
    def create_widgets(self):
        ...
        buttons = [
            ("Load from Docx/Pdf (Ctrl+L)", self.load_from_documentpdf),
    ...
    def calculate_totals(self):
        try:
            ...
            ending_cash_bank = self.safe_decimal(self.cash_bank_beg)
    ...
    def load_from_docx(self, filename):
        ...
        # Dictionary to map labels to variables
            label_to_var = {
                "Cash in Bank-beg": self.cash_bank_beg,
                "Cash on Hand-beg": self.cash_hand_beg,
                "Monthly dues collected": self.monthly_dues,
                "Certifications issued": self.certifications,
                "Membership fee": self.membership_fee,
                "Vehicle stickers": self.vehicle_stickers,
                ...
                "Livelihood Management Fee": self.livelihood_fee,
                "Others(inflow)": self.inflows_others,
                ...
                "Refund for seri-culture": self.refund_sericulture,
                "Others(outflow)": self.outflows_others,
                "Ending cash balance": self.ending_cash,
                "Cash in Bank": self.ending_cash_bank,
                "Cash on Hand": self.ending_cash_hand
            }
            ...
            for table in doc.tables:
                for row in table.rows:
                    if len(row.cells) >= 2:
                        ...
                        print(f"Extracted Label: '{label}', Value: '{value}'")  # Debugging
                    
                        # Special case for title and date
                        if label.lower().startswith("for the year month"):
                            try:
                                date_str = label.split("month")[1].strip()
                                self.date_var.set(self.parse_date_from_display(date_str))
                            except:
                                pass
                    
                    # Update the corresponding variable if label matches
                        if label in label_to_var:
                            label_to_var[label].set(parse_amount(value))
        
        # Update totals after loading all values
    ...
    def parse_date_from_display(self, display_date):
        # Implement this based on your date format
        # Example: "January 2023" to your internal date format
        try:
            from datetime import datetime
            return datetime.strptime(display_date, "%B %Y").strftime("%m/%Y")
        except:
            return display_date
    ...
    def load_from_pdf(self, filename):
        ...
        # Dictionary to map PDF labels to variables
            label_to_var = {
            "Cash in Bank-beg": self.cash_bank_beg,
            "Cash on Hand-beg": self.cash_hand_beg,
            "Monthly dues collected": self.monthly_dues,
            ...
            "Interest Income on bank deposits": self.interest_income,
            "Livelihood Management Fee": self.livelihood_fee,
            "Others(inflow)": self.inflows_others,  # Updated label
            ...
            "Withholding tax on bank deposit": self.withholding_tax,
            "Refund for seri-culture": self.refund_sericulture,
            "Others(outflow)": self.outflows_others,  # Updated label
            "Ending cash balance": self.ending_cash,
            "Cash in Bank": self.ending_cash_bank,
            "Cash on Hand": self.ending_cash_hand
        }
        ...
        # Update title and date from the PDF
            with pdfplumber.open(filename) as pdf:
        ...
        try:
            ...
            if filename.endswith('.pdf'):
                ...
                try:
                    ...
                    ("Unsupported file format. Please select a PDF or DOCX file.")
    ...
    def export_to_pdf(self):
        try:
            ...
            inflows_data = [
                ...
                ["Others(inflow)", format_amount(self.inflows_others.get())],
            ...
            outflows_data = [
                ...
                ["Others(outflow)", format_amount(self.outflows_others.get())]
            ]
