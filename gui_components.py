...
from email_attachments_window import EmailAttachmentsWindow 
...
class GUIComponents:
    def __init__(self, root, variables, title_var, date_var, display_date, calculator, file_handler, email_sender, settings_manager):
        ...
        self.required_vars = [
            ...
            'withdrawal_from_bank', 'inflows_others', # ADDED withdrawal_from_bank
            'snacks_meals', 'transportation', 'office_supplies',
        ...
        self.FOOTER_BG = "#192337" 
        self.REMOVE_BTN_FG_COLOR = "#D32F2F" 
        self.REMOVE_BTN_HOVER_COLOR = "#C62828"
        self.ATTACH_BTN_FG_COLOR = "#FF9800" 
        ...
        self.base_font_size = max(9, min(15, int(self.screen_height / 70))) 
        ...
        if 'logo_path_var' in self.variables: 
    ...
    def _initialize_missing_variables(self):
        ...
        for var_key in self.required_vars:
            if var_key not in self.variables:
                ...
                elif var_key == 'logo_path_var': 
                    default_value = ""
                elif var_key == 'recipient_emails_var': 
                    default_value = ""
                # ADDED default for withdrawal_from_bank (though it will be blank string)
                elif var_key == 'withdrawal_from_bank':
                    default_value = ""
    ...
    def _update_logo_display(self, *args):
        ...
        if logo_path:
            if os.path.exists(logo_path):
                ...
                display_text = "Logo file not found!" 
        ...
        def check_queue():
            try:
                ...
                if isinstance(response, dict):
                    ...
                    if status == "success":
                        if message: messagebox.showinfo("Success", message)
                        logging.info(f"Action '{action_name}' completed: {message or 'OK'}")
                    elif status == "error":
                        if message: messagebox.showerror("Error", message)
                        else: messagebox.showerror("Error", f"An error occurred during {action_name}.")
                        ...
                        if message: messagebox.showinfo("Info", f"{action_name} finished.\nStatus: {status}\nMessage: {message}")
                        else: messagebox.showinfo("Info", f"{action_name} finished.\nStatus: {status}")
                else:
                    logging.warning(f"Legacy or unexpected return type from {action_name}: {response}")
                    if response not in [None, False]: messagebox.showinfo("Completed", f"{action_name} finished successfully.")
            except queue.Empty:
                if loading_window.winfo_exists():
                    self.root.after(100, check_queue)
            except Exception as e:
                if loading_window.winfo_exists(): loading_window.close_window()
    ...
    def show_calendar(self):
        ...
        popup.transient(self.root); popup.grab_set(); popup.attributes("-topmost", True)
        try:
            ...
            except ValueError: logging.warning(f"Could not pre-select date '{current_date_str}' in calendar.")
            def on_date_select():
                selected_date = cal.selection_get()
                if selected_date: self.date_var.set(selected_date.strftime("%m/%d/%Y")); logging.info(f"Date selected from calendar: {self.date_var.get()}")
                else: logging.warning("Calendar closed without selection.")
                popup.destroy()
            confirm_button_width = min(max(100, int(popup_width * 0.3)), 160)
            confirm_button = ctk.CTkButton(popup, text="Confirm", command=on_date_select, font=("Roboto", self.button_font_size), width=confirm_button_width, corner_radius=8, fg_color=self.BUTTON_FG_COLOR, hover_color=self.BUTTON_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR)
    ...
    def _select_logo(self):
        ...
        filepath = filedialog.askopenfilename(title="Select Logo Image", filetypes=filetypes)
        if filepath:
        ...
        else: logging.info("Logo selection cancelled.")
    ...
    def show_email_attachments_window(self):
        ...
        if 'recipient_emails_var' not in self.variables:
             ...
             return
        EmailAttachmentsWindow(self.root, self.email_sender, self.variables['recipient_emails_var'])
    ...
    def create_widgets(self):
        ...
        self.content_frame.grid(row=0, column=0, sticky="nsew", padx=10, pady=(10, 10)) 
        ...
        compliance_label = ctk.CTkLabel(copyright_github_container, text="© 2025 All Rights Reserved", font=("Roboto", 10), text_color="#FFFFFF", justify="right")
        ...
        try:
            ...
            if os.path.exists(gh_logo_path):
                gh_logo_pil = Image.open(gh_logo_path).convert("RGBA"); gh_logo_size = (16, 16); gh_logo_pil = gh_logo_pil.resize(gh_logo_size, Image.Resampling.LANCZOS)
                ...
                gh_logo_label.pack(side="left", padx=(8, 0)); gh_logo_label.bind("<Button-1>", lambda e, url=github_url: webbrowser.open_new_tab(url))
            else:
                ...
                github_fallback_label.pack(side="left", padx=(5, 0)); github_fallback_label.bind("<Button-1>", lambda e, url=github_url: webbrowser.open_new_tab(url))
        ...
        try:
            ...
            if os.path.exists(image1_path):
                ...
                ctk.CTkLabel(self.footer_frame, image=ctk_img1, text="").pack(side="left", padx=(20, 10), pady=self.base_pad_y)
            else: ctk.CTkLabel(self.footer_frame, text="Img1 N/A", font=("Roboto", self.label_font_size-2), text_color="#FFFFFF").pack(side="left", padx=(20, 10), pady=self.base_pad_y)
            if os.path.exists(image2_path):
                ...
                ctk.CTkLabel(self.footer_frame, image=ctk_img2, text="").pack(side="left", padx=(10, 20), pady=self.base_pad_y)
            else: ctk.CTkLabel(self.footer_frame, text="Img2 N/A", font=("Roboto", self.label_font_size-2), text_color="#FFFFFF").pack(side="left", padx=(10, 20), pady=self.base_pad_y)
        except Exception as e:
        ...
        ctk.CTkEntry(address_frame, textvariable=self.variables['address_var'], font=("Roboto", self.entry_font_size), corner_radius=6, fg_color=self.ENTRY_BG_COLOR, text_color=self.TEXT_COLOR, border_color=self.ENTRY_BORDER_COLOR).pack(side="top", fill="x")
        ...
        ctk.CTkButton(logo_buttons_frame, text="Select Logo", font=("Roboto", self.button_font_size), command=self._select_logo, corner_radius=6, fg_color=self.BUTTON_FG_COLOR, hover_color=self.BUTTON_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR).pack(side="left", padx=(0, self.base_pad_x // 2))
        ctk.CTkButton(logo_buttons_frame, text="Remove Logo", font=("Roboto", self.button_font_size), command=self._remove_logo, corner_radius=6, fg_color=self.REMOVE_BTN_FG_COLOR, hover_color=self.REMOVE_BTN_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR).pack(side="left", padx=(0, self.base_pad_x))
        ...
        buttons_subframe.pack(side="top", fill="x", anchor="w") 
        settings_button_width = min(max(120, int(self.screen_width * 0.08)), 180)
        ctk.CTkButton(buttons_subframe, text="Account Settings", font=("Roboto", self.button_font_size), command=self.show_settings, corner_radius=6, fg_color=self.BUTTON_FG_COLOR, hover_color=self.BUTTON_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR, width=settings_button_width).pack(side="left", padx=(0, self.base_pad_x // 2))
        attachments_button_width = min(max(120, int(self.screen_width * 0.1)), 200)
        ctk.CTkButton(buttons_subframe, text="Manage Attachments & Send", font=("Roboto", self.button_font_size), command=self.show_email_attachments_window, corner_radius=6, fg_color=self.ATTACH_BTN_FG_COLOR, hover_color=self.ATTACH_BTN_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR, width=attachments_button_width).pack(side="left", padx=(self.base_pad_x // 2, 0))
        ...
        ctk.CTkButton(date_frame, textvariable=self.display_date, font=("Roboto", self.button_font_size), command=self.show_calendar, corner_radius=6, fg_color=self.DATE_BTN_FG, hover_color=self.DATE_BTN_HOVER, text_color=self.DATE_BTN_TEXT, width=date_button_width).pack(side="top", anchor="w")

        button_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
        button_frame.grid(row=1, column=0, sticky="ew", padx=self.section_pad_x, pady=(self.base_pad_y, self.section_pad_y // 2)) 
        buttons_data = [("Load (Ctrl+L)", self.file_handler.load_from_documentpdf, "Load data from DOCX/PDF"), ("Clear Fields", self.clear_fields, "Clear all input fields"), ("Export PDF (Ctrl+E)", self.file_handler.export_to_pdf, "Export as PDF"), ("Save Word (Ctrl+W)", self.file_handler.save_to_docx, "Save as DOCX")]
        num_buttons = len(buttons_data)
        if num_buttons > 0: button_frame.grid_columnconfigure(tuple(range(num_buttons)), weight=1, uniform="button_group")
        for i, (text, command, tooltip_text) in enumerate(buttons_data):
            action_name_simple = text.split('(')[0].strip(); loading_message = f"{action_name_simple}..."
            actual_command = (lambda f=command, name=action_name_simple, msg=loading_message: self._execute_with_loading(f, name, msg)) if command in [self.file_handler.export_to_pdf, self.file_handler.save_to_docx] else (lambda cmd=command, name=action_name_simple: self._safe_call(cmd, name))
            ctk.CTkButton(button_frame, text=text, command=actual_command, font=("Roboto", self.button_font_size), corner_radius=8, fg_color=self.BUTTON_FG_COLOR, hover_color=self.BUTTON_HOVER_COLOR, text_color=self.BUTTON_TEXT_COLOR, height=int(self.base_font_size * 2.5)).grid(row=0, column=i, sticky="ew", padx=self.base_pad_x // 4, pady=self.base_pad_y // 4)

        self.columns_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
        self.columns_frame.grid(row=2, column=0, sticky="ew", padx=self.section_pad_x // 2, pady=(self.base_pad_y, self.section_pad_y)) 
        num_data_cols = 5; min_col_width = int(self.min_input_width * 1.7) 
        for i in range(num_data_cols): self.columns_frame.grid_columnconfigure(i, weight=1, uniform="data_cols", minsize=min_col_width)
        self.beg_frame = self._create_section_frame("Beginning Cash Balances"); self.inflow_frame = self._create_section_frame("Cash Inflows"); self.outflow_frame = self._create_section_frame("Cash Outflows"); self.end_frame = self._create_section_frame("Ending Cash Balances (Calculated)"); self.totals_frame = self._create_section_frame("Totals (Calculated)")
        self.beg_frame.grid(row=0, column=0, sticky="nsew", padx=self.base_pad_x//2, pady=self.base_pad_y); self.inflow_frame.grid(row=0, column=1, sticky="nsew", padx=self.base_pad_x//2, pady=self.base_pad_y); self.outflow_frame.grid(row=0, column=2, sticky="nsew", padx=self.base_pad_x//2, pady=self.base_pad_y); self.end_frame.grid(row=0, column=3, sticky="nsew", padx=self.base_pad_x//2, pady=self.base_pad_y); self.totals_frame.grid(row=0, column=4, sticky="nsew", padx=self.base_pad_x//2, pady=self.base_pad_y)
        ...
        names_frame.grid( row=3, column=0, sticky="ew", padx=self.section_pad_x, pady=(0, self.base_pad_y) ) 
        name_fields_data = [("Prepared by (Treasurer):", 'prepared_by_var', "Name of HOA Treasurer"), ("Noted by (President):", 'noted_by_var_1', "Name of HOA President"), ("Noted by (CHUDD HCD-CORDS):", 'noted_by_var_2', "Name of CHUDD HCD-CORDS rep"), ("Checked by (Auditor):", 'checked_by_var', "Name of HOA Auditor")]
        num_name_fields = len(name_fields_data); min_name_col_width = int(self.min_input_width * 1.5) 
        for i in range(num_name_fields): names_frame.grid_columnconfigure(i, weight=1, uniform="name_group_adjusted", minsize=min_name_col_width)
        for i, (label_text, var_key, tooltip_text) in enumerate(name_fields_data):
            frame = ctk.CTkFrame(names_frame, fg_color="transparent"); frame.grid(row=0, column=i, sticky="nsew", padx=self.base_pad_x, pady=self.base_pad_y); frame.grid_columnconfigure(0, weight=1)
            ...
            ctk.CTkEntry(frame, textvariable=self.variables[var_key], font=("Roboto", self.entry_font_size), corner_radius=6, fg_color=self.ENTRY_BG_COLOR, text_color=self.TEXT_COLOR, border_color=self.ENTRY_BORDER_COLOR).grid(row=1, column=0, sticky="ew")
    ...
    def _create_section_frame(self, title):
        ...
        ctk.CTkLabel(frame, text=title, font=("Roboto", self.title_font_size, "bold"), text_color=self.TEXT_COLOR, anchor="w").pack(fill="x", padx=self.base_pad_x * 1.5, pady=(self.base_pad_y * 1.5, self.base_pad_y))
        ctk.CTkFrame(frame, fg_color="transparent").pack(fill="both", expand=True, padx=self.base_pad_x, pady=(0, self.base_pad_y * 1.5))
        return frame
    ...
    def _create_entry_pair(self, parent_frame, label_text, var, is_disabled=False, tooltip_text=None, input_width=None, is_numeric=False):
        ...
        ctk.CTkLabel(item_frame, text=label_text, font=("Roboto", self.label_font_size), text_color=self.TEXT_COLOR, anchor="w").grid(row=0, column=0, sticky="w", padx=(0, self.base_pad_x))
        preferred_entry_width = input_width or self.min_input_width
        entry_config = {"textvariable": var, "width": preferred_entry_width, "font": ("Roboto", self.entry_font_size), "corner_radius": 6, "fg_color": self.DISABLED_BG_COLOR if is_disabled else self.ENTRY_BG_COLOR, "text_color": self.TEXT_COLOR, "border_color": self.ENTRY_BORDER_COLOR, "state": "disabled" if is_disabled else "normal", "justify": "right"}
        if is_numeric and not is_disabled: entry_config["validate"] = "key"; entry_config["validatecommand"] = self.vcmd_numeric
    ...
    def populate_columns(self):
        ...
        # --- MODIFIED inflow_items to include Withdrawal from Bank ---
        inflow_items = [
            ("Monthly dues collected:", 'monthly_dues'), 
            ("Certifications issued:", 'certifications'), 
            ("Membership fee:", 'membership_fee'), 
            ("Vehicle stickers:", 'vehicle_stickers'), 
            ("Rentals:", 'rentals'), 
            ("Solicitations/Donations:", 'solicitations'), 
            ("Interest Income:", 'interest_income'), 
            ("Livelihood Fee:", 'livelihood_fee'), 
            ("Withdrawal from Bank:", 'withdrawal_from_bank', "Cash withdrawn from bank to hand"), # ADDED
    ...
    def update_layout(self):
        if self.main_frame.winfo_exists(): self.main_frame.update_idletasks()
        if self.content_frame.winfo_exists(): self.content_frame.update_idletasks() 
    ...
    def clear_fields(self):
        ...
        if not messagebox.askyesno("Confirm Clear", "Clear all input and calculated fields?\n(Logo, Address, Date, and Signatories will remain)"):
            return
        cleared_count = 0
        fields_to_keep = {'logo_path_var', 'address_var', 'date_var', 'display_date', 'recipient_emails_var', 'prepared_by_var', 'noted_by_var_1', 'noted_by_var_2', 'checked_by_var', 'title_var', 'footer_image1_var', 'footer_image2_var'}
        for key, var in self.variables.items():
            if key not in fields_to_keep and isinstance(var, ctk.StringVar):
                try: var.set(""); cleared_count += 1
                except tkinter.TclError as e: logging.warning(f"Could not clear variable '{key}' (might be destroyed): {e}")
        if 'recipient_emails_var' in self.variables: self.variables['recipient_emails_var'].set(""); logging.info("Cleared recipient email variable.")
        if hasattr(self, 'email_sender') and hasattr(self.email_sender, 'attachments'):
            if self.email_sender.attachments: self.email_sender.attachments.clear(); logging.info("Cleared additional email attachments list.")
        messagebox.showinfo("Success", "Cash flow data fields, recipient list, and additional attachments have been cleared.")
        try:
            if hasattr(self.calculator, 'calculate_totals'): self.calculator.calculate_totals()
            else: logging.warning("Calculator has no 'calculate_totals' method to call after clearing.")
        except Exception as e: logging.exception("Error recalculating totals after clearing fields.")
...
if __name__ == '__main__':
    ...
    class MockFileHandler:
        ...
        def export_to_pdf(self): time.sleep(1); return {"status": "success", "message": "Mock PDF Exported!"}
        def save_to_docx(self): time.sleep(1); return {"status": "success", "message": "Mock Word Saved!"}
    class MockEmailSender:
        def __init__(self, settings_manager, recipient_emails_var, file_handler): self.settings_manager = settings_manager; self.recipient_emails_var = recipient_emails_var; self.file_handler = file_handler; self.attachments = []
        def send_email(self):
            time.sleep(1.5); recipients = self.recipient_emails_var.get()
    ...
    root = ctk.CTk(); root.title("HOA Cash Flow (Test)"); root.geometry("1200x750"); root.resizable(True, True); root.minsize(700, 500)
    settings_manager = SettingsManager(); variables = {}; title_var = ctk.StringVar(value="HOA Cash Flow Statement"); date_var = ctk.StringVar(value=datetime.date.today().strftime("%m/%d/%Y")); display_date = ctk.StringVar()
    calculator = MockCalculator(); file_handler = MockFileHandler()
    gui = GUIComponents(root, variables, title_var, date_var, display_date, calculator, file_handler, None, settings_manager)
    if 'recipient_emails_var' in gui.variables: email_sender = MockEmailSender(settings_manager, gui.variables['recipient_emails_var'], file_handler); gui.email_sender = email_sender
    else: messagebox.showerror("Test Init Error", "recipient_emails_var was not initialized!"); root.destroy(); exit()
    root.after(200, gui.update_layout); root.mainloop()
